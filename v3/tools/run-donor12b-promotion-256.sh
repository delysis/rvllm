#!/bin/sh
# One case per queue job so interleaved route comparisons have independent receipts.
set -eu
[ "$#" -eq 4 ] || { echo 'usage: run-donor12b-promotion-256.sh INFER SELECTOR METALLIB REPORT' >&2; exit 64; }
infer=$1
selector=$2
metallib=$3
report=$4
...
selected=$(mktemp /tmp/rvllm-donor12b-promotion-256.XXXXXX)
trap 'rm -f -- "$selected"' EXIT
sed -n '1p' "$prompts" > "$selected"
...
RVLLM_METAL_RESEARCH="$selector" RVLLM_METAL_METALLIB_BF16="$metallib" \
  ...
  --session-backend direct --max-new-tokens 2 --max-total-tokens 258 \
