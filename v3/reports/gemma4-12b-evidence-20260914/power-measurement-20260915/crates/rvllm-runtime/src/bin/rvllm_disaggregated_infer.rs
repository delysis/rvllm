...
use rvllm_runtime::gemma_ane_decode::{
    provision_static_cache_with_capacity, AneStaticCachePart, AneWeightPlan, GemmaAneDecode,
};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(flag) = args.next() {
        ...
        match flag.as_str() {
            ...
            "--prepare-ane-cache" => {
                prepare_cache = Some(match value.as_str() {
                    ...
                    _ => {
                        return Err(
                            "--prepare-ane-cache must be qkv, output, ffn, ffn-int8, ffn-lut4 or head-attention".into(),
                        )
                    }
                });
            }
            "--ane-weights" => {
                ane_weights =
                    match value.as_str() {
                        ...
                        _ => return Err(
                            "--ane-weights must be dynamic-ffn, static-ffn, static-all-cached, static-int8-ffn-cached or static-lut4-ffn-cached"
                                .into(),
                        ),
                    }
            }
    ...
    if let Some(part) = prepare_cache {
        if !paths.is_empty() || !prompts.is_empty() || capture_layers || interactive || interleave {
            return Err(
                "cache provisioning cannot be combined with inference references or layer capture"
                    .into(),
            );
        }
        ...
        let programs = provision_static_cache_with_capacity(&model_dir, part, context_capacity)?;
        let report = serde_json::json!({
            "schema":"rvllm.ane_cache_provision.v1", "part":format!("{part:?}"),
            "model_dir":model_dir, "programs_prepared":programs,
            "maximum_explicit_compiles":programs, "cache_policy":"reuse-or-compile",
            ...
            "claim":"Bounded provisioning only; cache entries can later be evicted. This is not full residency or numerical qualification."
        });
        std::fs::write(
    ...
    if (!paths.is_empty() || capture_layers) && output_dir.is_none() {
    ...
    loop {
        ...
        let case_dir = output_dir
            .as_ref()
            .map(|dir| dir.join(format!("case-{index}")));
        let (start, prefill_sample_capture_ms, prefill_measurement) = if interleave {
            prefill_case(
                &monitor,
                retained_metal
                    .as_mut()
        ...
        if generated.len() < reference.max_new_tokens && !eos.contains(&generated[0]) {
                ...
                && !eos.contains(generated.last().ok_or("missing token")?)
            {
                ...
                let measured = monitor.begin();
                let result = if capture_layers && generated.len() == 1 {
                    decoder.decode_with_observer(input, &mut |layer, values| {
...
fn prefill_case(
    ...
    capture_seed: bool,
) -> Result<(AneDecodeStart, f64, serde_json::Value), Box<dyn std::error::Error>> {
    if let Some(directory) = case_dir {
        std::fs::create_dir(directory)?;
    }
    let owner = ReqId(1);
    ...
    Ok((start, prefill_sample_capture_ms, measurement))
}
