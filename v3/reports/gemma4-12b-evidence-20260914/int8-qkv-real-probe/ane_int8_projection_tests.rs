...
#[test]
#[ignore = "actual Gemma QKV: explicit model/capture/output/mode; cpu has zero device calls, qualify at most two compiles; time strictly cached"]
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        return Err(
            "requires layer 1..47, 1..8 previous-layer captures, mode cpu|qualify|time".into(),
        );
    }
    let journal = std::env::var_os("RVLLM_ANE_DIAGNOSTIC_JOURNAL").is_some();
    if (mode == "qualify" && !journal) || (mode == "time" && journal) {
        return Err("qualify requires the driver journal; time forbids it".into());
    }
