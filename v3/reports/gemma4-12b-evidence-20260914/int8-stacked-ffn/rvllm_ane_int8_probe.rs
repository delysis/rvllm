...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let int8_layout_experiment = int8_lut_storage || int8_stacked;
    if (int8_layout_experiment && mode != Mode::Int8) || (int8_lut_storage && int8_stacked) {
        return Err("select one INT8 layout experiment and --mode int8".into());
    }
    if compile_budget > 2 || (compile_budget != 0 && (!int8_layout_experiment || cpu_only)) {
        return Err(
            "--ane-compile-budget 0..=2 applies only to an INT8 layout hardware comparison".into(),
        );
    }
    ...
    {
        return Err("stacked INT8 qualification requires the driver journal".into());
    }
    ...
    let reconstructed = if int8_layout_experiment {
    ...
    if cpu_only {
        ...
        let result = json!({"schema":"rvllm.ane_ffn_host_quality.v1", "model_dir":model_dir,"layer":layer,
            "mode":mode.name(),"int8_lut_storage":int8_lut_storage,"int8_stacked":int8_stacked,"exact_int8_weight_parity":if int8_layout_experiment { Some(true) } else { None },"accelerator_calls":0,"compiler_calls":0,
