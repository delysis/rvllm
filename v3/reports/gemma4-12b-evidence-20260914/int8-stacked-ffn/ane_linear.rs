...
impl AneGatedFfn {
    ...
    pub fn project(&mut self, input: &[f16], output: &mut [f16]) -> Result<(), String> {
        if input.len() != self.hidden || output.len() != self.hidden {
...
fn ffn_mil_with_constants(hidden: usize, intermediate: usize, weights: &str) -> String {
    let projections = format!(
        r#"        tensor<fp16, [1, {intermediate}, 1, 1]> gate = conv(dilations = dilations, groups = groups, pad = pad, pad_type = pad_type, strides = strides, weight = Wg, x = x)[name = string("gate")];
        tensor<fp16, [1, {intermediate}, 1, 1]> up = conv(dilations = dilations, groups = groups, pad = pad, pad_type = pad_type, strides = strides, weight = Wu, x = x)[name = string("up")];"#
    );
    ffn_mil_with_projections(hidden, intermediate, weights, &projections)
}
...
[buildInfo = dict<string, string>({{{{"coremlc-component-MIL", "3510.2.1"}}, {{"coremlc-version", "3505.4.1"}}, {{"coremltools-version", "9.0"}}}})]
{{
    func main<ios18>(tensor<fp16, [1, {hidden}, 1, 1]> x) {{
        string pad_type = const()[name = string("pad_type"), val = string("valid")];
        tensor<int32, [2]> strides = const()[name = string("strides"), val = tensor<int32, [2]>([1, 1])];
        tensor<int32, [4]> pad = const()[name = string("pad"), val = tensor<int32, [4]>([0, 0, 0, 0])];
        tensor<int32, [2]> dilations = const()[name = string("dilations"), val = tensor<int32, [2]>([1, 1])];
...
{weights}
{projections}
        tensor<fp16, [1, {intermediate}, 1, 1]> gate_squared = mul(x = gate, y = gate)[name = string("gate_squared")];
        tensor<fp16, [1, {intermediate}, 1, 1]> gate_cubed = mul(x = gate_squared, y = gate)[name = string("gate_cubed")];
        tensor<fp16, [1, {intermediate}, 1, 1]> cubic_scaled = mul(x = gate_cubed, y = gelu_cubic)[name = string("cubic_scaled")];
        tensor<fp16, [1, {intermediate}, 1, 1]> gelu_sum = add(x = gate, y = cubic_scaled)[name = string("gelu_sum")];
        tensor<fp16, [1, {intermediate}, 1, 1]> gelu_argument = mul(x = gelu_sum, y = gelu_root)[name = string("gelu_argument")];
        tensor<fp16, [1, {intermediate}, 1, 1]> gelu_tanh = tanh(x = gelu_argument)[name = string("gelu_tanh")];
        tensor<fp16, [1, {intermediate}, 1, 1]> gelu_factor = add(x = gelu_tanh, y = gelu_one)[name = string("gelu_factor")];
        tensor<fp16, [1, {intermediate}, 1, 1]> gate_halved = mul(x = gate, y = gelu_half)[name = string("gate_halved")];
        tensor<fp16, [1, {intermediate}, 1, 1]> activated = mul(x = gate_halved, y = gelu_factor)[name = string("activated")];
        tensor<fp16, [1, {intermediate}, 1, 1]> gated = mul(x = activated, y = up)[name = string("gated")];
        tensor<fp16, [1, {hidden}, 1, 1]> y = conv(dilations = dilations, groups = groups, pad = pad, pad_type = pad_type, strides = strides, weight = Wd, x = gated)[name = string("down")];
    }} -> (y);
}}
"#
    )
