...
impl AneGatedFfn {
    ...
    pub fn project_batch(&mut self, input: &[f16], output: &mut [f16]) -> Result<(), String> {
        ...
        pack_ffn_batch(input, self.hidden, self.logical_spatial, &mut self.input)?;
        self.kernel.write_input(&self.input)?;
        self.kernel.evaluate()?;
        self.kernel.read_output(&mut self.output)?;
        for (token, values) in output.chunks_exact_mut(self.hidden).enumerate() {
            for (channel, value) in values.iter_mut().enumerate() {
                let offset = (channel * 32 + token) * 2;
                *value = f16::from_le_bytes([self.output[offset], self.output[offset + 1]]);
            }
        }
        Ok(())
...
fn pack_ffn_batch(
    ...
    {
        return Err("ANE FFN packed batch shape mismatch".into());
    }
...
mod tests {
    ...
    fn batch_ffn_packing_keeps_distinct_columns_and_erases_padding() {
        ...
        pack_ffn_batch(&input, 3, 2, &mut packed).unwrap();
        ...
        pack_ffn_batch(&[f16::ZERO; 6], 3, 2, &mut packed).unwrap();
        assert!(packed.iter().all(|&byte| byte == 0));
        for (hidden, tokens) in [(0, 2), (3, 0), (3, 9), (usize::MAX, 2), (3, 3)] {
            assert!(pack_ffn_batch(&input, hidden, tokens, &mut packed).is_err());
            assert!(packed.iter().all(|&byte| byte == 0));
        }
    }
