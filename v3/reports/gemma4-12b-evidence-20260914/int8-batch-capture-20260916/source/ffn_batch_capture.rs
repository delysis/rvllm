...
pub(super) fn run(
    ...
    for ((input, output), reference) in inputs.iter().zip(&mut serial_outputs).zip(&references) {
        serial.project(input, output)?;
        serial_records.push(output_record(output, reference)?);
    }
    drop(serial);
    ...
    Ok(
        ...
        "affine_coefficients_sha256":ffn_oracle::affine_hash(weights),"reconstructed_fp16_sha256":reconstructed_hash,
