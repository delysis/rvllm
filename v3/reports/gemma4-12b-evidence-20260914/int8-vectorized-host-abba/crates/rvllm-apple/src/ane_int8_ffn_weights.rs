...
impl RowInt8 {
    // Host-only candidate. Keep the scalar path until source-byte parity and
    // power-stratified CPU measurements qualify this implementation.
...
impl AneInt8FfnWeights {
    ...
    /// Experimental host preparation only; no ANE calls or default-route change.
    /// Compare against `quantize` before using the result as a source artifact.
    pub fn quantize_vectorized_candidate(
        gate: &[f16],
        up: &[f16],
        down: &[f16],
        hidden: usize,
...
mod tests {
    ...
    fn vectorized_quantizer_matches_gemma_row_widths_and_source_blob() {
        ...
        let weights: Vec<_> = (0..32 * 64)
            ...
            .collect();
        let scalar = AneInt8FfnWeights::quantize(&weights, &weights, &weights, 32, 64).unwrap();
        let vector =
            AneInt8FfnWeights::quantize_vectorized_candidate(&weights, &weights, &weights, 32, 64)
                .unwrap();
