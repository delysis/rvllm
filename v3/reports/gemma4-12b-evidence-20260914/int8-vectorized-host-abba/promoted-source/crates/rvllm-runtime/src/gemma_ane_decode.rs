...
impl AneWeightPlan {
    ...
    pub fn program_count(self) -> usize {
        match self {
            ...
            Self::StaticAllCached | Self::StaticLut4FfnCached | Self::StaticInt8FfnCached => 162,
        }
    }

    pub fn cache_policy(self) -> AneProgramCachePolicy {
        match self {
            Self::StaticAllCached | Self::StaticLut4FfnCached | Self::StaticInt8FfnCached => {
    ...
    fn static_ffn_precision(self) -> StaticFfnPrecision {
        match self {
            Self::StaticInt8FfnCached => StaticFfnPrecision::Int8,
...
pub fn provision_static_cache_with_capacity(
    ...
    let policy = AneProgramCachePolicy::ReuseOrCompile;
    if part == AneStaticCachePart::VocabularyAndAttention {
        drop(AneAttentionProgram::compile_sliding_with_cache_policy(
            16, 8, 256, 1024, policy,
        )?);
        drop(AneAttentionProgram::compile_with_cache_policy(
            16, 1, 512, capacity, policy,
        )?);
        ...
        for first in (0..VOCAB).step_by(HEAD_ROWS) {
            ...
            drop(AneLinear::compile_with_cache_policy(
                &weights, HIDDEN, HEAD_ROWS, 1, policy,
            )?);
            eprintln!(
                "ANE cache prepared vocabulary rows {first}..{}",
                first + HEAD_ROWS
            );
        }
        return Ok(2 + VOCAB / HEAD_ROWS);
    }
    for index in 0..LAYERS {
        ...
        eprintln!("ANE cache prepared {part:?} layer {index}/{}", LAYERS - 1);
    }
    Ok(LAYERS)
}
...
impl GemmaAneDecode {
    ...
    pub fn load_with_compile_budget(
        ...
        for index in 0..LAYERS {
            ...
            let qkv = AneLinear::compile_with_cache_policy(
                &qkv_weights,
                HIDDEN,
                projection_width,
                1,
    ...
    pub fn decode_with_observer(
        ...
        self.next_position = None;
        let result = self.decode_inner(token, position, observer)?;
