...
if (( $# != 2 )); then
  print -u2 'usage: run-baab-v2.sh LENGTH OUTPUT_DIR'
  exit 64
fi
...
mkdir "$output_dir"
tmp=$(mktemp -d /tmp/rvllm-split32-baab-v2.XXXXXX)
trap 'rm -rf -- "$tmp"' EXIT
...
arms=(candidate control control candidate)
for index in {1..4}; do
  arm=${arms[$index]}
  if [[ $arm == control ]]; then
    selector=$control
...
jq -n \
  ...
  --slurpfile b1 "$output_dir/1-candidate.json" \
  --slurpfile a1 "$output_dir/2-control.json" \
  --slurpfile a2 "$output_dir/3-control.json" \
  --slurpfile b2 "$output_dir/4-candidate.json" '
  ...
  {
    schema: "rvllm.gemma4.split32-normal-route-baab.v2",
    length: $length,
