...
| multi-step decode selected logits | REAL-CORRECTNESS | `cpu_reference_decode_loop_selected_logits_sequence_is_expected`, `engine_decode_loop_selected_logits_match_cpu` | Yes, ignored Metal test | Selected logits only | Full per-step logits |
| generated tiny Gemma-shaped selected logits | REAL-CORRECTNESS | `tiny_generated_gemma4_hf_end_to_end_model_backend_selected_logits_match_cpu` | Yes, ignored Metal test | Synthetic generated HF/Gemma-shaped fixture; selected logits only | Full per-step logits |
