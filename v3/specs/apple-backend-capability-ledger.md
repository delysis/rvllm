...
| multi-head grouped-KV attention | SYNTHETIC-NUMERIC | `cpu_reference_multihead_gqa_attention_full_logits_are_stable`, `tiny_multihead_gqa_attention_model_backend_full_logits_match_cpu`, `engine_multihead_gqa_attention_full_logits_match_cpu` | Yes, ignored Metal tests | Synthetic one-token full-logit evidence; no real checkpoint decode | Broaden to generated HF-shaped multi-layer GQA and real checkpoint dry-run |
| q_dim distinct from hidden | SYNTHETIC-NUMERIC | `cpu_reference_qdim_not_hidden_full_logits_are_stable`, `tiny_qdim_not_hidden_model_backend_full_logits_match_cpu`, `engine_qdim_not_hidden_full_logits_match_cpu` | Yes, ignored Metal tests | Synthetic one-token full-logit evidence; no real checkpoint decode | Broaden to generated HF-shaped q_dim != hidden and real checkpoint dry-run |
...
| generated tiny reference export bundle | GENERATED-HF-NUMERIC | `generated_tiny_hf_reference_bundle_can_be_exported`, `scripts/verify_generated_tiny_reference.py` | No | Env-gated export hook plus standalone verifier; synthetic generated fixture only | Compare exported bundle against PyTorch or another external implementation |
