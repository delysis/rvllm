...
| Gemma dry-run/load arch contract | REAL-CHECKPOINT-DRYRUN | `gemma4_arch::*`, `gemma4_dry_run_validates_text_config_and_model_language_model_prefix`, generated tiny dry-run validation | No Metal execution required | Dry-run now derives layer kinds, dimensions, and prefix from the Gemma4 loader arch path; still metadata-only | Run against real model dirs, including sharded and single-shard checkpoints |
