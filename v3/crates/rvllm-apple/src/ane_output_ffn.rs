...
{weights}        tensor<fp16, [1, {hidden}, 1, 1]> post_gamma = const()[val = tensor<fp16, [1, {hidden}, 1, 1]>([{post_gamma}])];
        ...
        tensor<fp16, [1, 1, 1, 1]> post_inv = rsqrt(x = add(x = mul(x = post_sum, y = norm_mean), y = norm_epsilon));
        tensor<fp16, [1, {hidden}, 1, 1]> post_norm = mul(x = mul(x = projected, y = post_inv), y = post_gamma);
        ...
        tensor<fp16, [1, 1, 1, 1]> pre_inv = rsqrt(x = add(x = mul(x = pre_sum, y = norm_mean), y = norm_epsilon));
        tensor<fp16, [1, {hidden}, 1, 1]> x = mul(x = mul(x = merged, y = pre_inv), y = pre_gamma);
        ...
        tensor<fp16, [1, {intermediate}, 1, 1]> ga = mul(x = add(x = gate, y = mul(x = g3, y = cubic)), y = root);
        tensor<fp16, [1, {intermediate}, 1, 1]> activated = mul(x = mul(x = gate, y = half), y = add(x = tanh(x = ga), y = one));
