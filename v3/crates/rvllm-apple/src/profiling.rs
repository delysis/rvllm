...
impl AppleProductionAcceptanceEvidence {
    ...
    pub fn current_real_e2b_probe() -> Self {
        ...
        Self {
            ...
            tokenizer_text_decoding: EvidenceState::missing(
                "diagnostic and bounded Metal CLIs can tokenize prompts and decode sampled/output token IDs, and three bounded reference-backed text smokes now match after correcting layer-scalar ordering; tokenizer/text decoding remains incomplete because coverage is still narrow and the workflow is capped by the probe arena rather than production serving",
            ),
