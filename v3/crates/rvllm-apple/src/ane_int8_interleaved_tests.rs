...
fn paired_rows_and_scales_preserve_exact_int8_identity_and_all_output_values() -> TestResult {
    ...
    for row in 0..g.scales.len() {
        for matrix in [&g, &u] {
            expected_q.extend(matrix.values[row * g.columns..(row + 1) * g.columns]
                .iter().map(|value| value.to_le_bytes()[0]));
    ...
    assert_eq!(dq, d.values.iter().map(|x| x.to_le_bytes()[0]).collect::<Vec<_>>());
    assert_eq!(ds, d.scales.iter().flat_map(|x| x.to_le_bytes()).collect::<Vec<_>>());
    let [gate, up, down] = weights.dequantized();
    for sample in 0..7 {
        let input = (0..32)
            .map(|n| f16::from_f32(((n * 7 + sample) % 29) as f32 / 32.0 - 0.4))
            .collect::<Vec<_>>();
        let gated = projection(&gate, &input).into_iter().zip(projection(&up, &input))
            .map(|(g, u)| activation(g, u)).collect::<Vec<_>>();
...
fn wrong_pair_slicing_is_observably_wrong_and_out_of_range_slicing_is_rejected() -> TestResult {
    ...
    let input = (0..32).map(|n| f16::from_f32((n % 11) as f32 / 8.0 - 0.5)).collect::<Vec<_>>();
...
fn paired_layout_admits_only_archived_geometry_and_retains_single_io() -> TestResult {
    ...
    for shape in [(0, 0), (32, 128), (3840, 15361), (3841, 15360), (usize::MAX, 15360)] {
    ...
    assert!(source.mil.contains("func main<ios18>(tensor<fp16, [1, 32, 1, 1]> x)"));
