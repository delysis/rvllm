...
fn unsupported_ops_and_duplicate_definitions_cannot_be_silently_skipped() -> TestResult {
    ...
    let line = original
        .lines()
        .find(|line| line.contains("fp16 gelu_half ="))
        .ok_or("scalar fixture")?;
...
fn actual_concat_and_convolution_arguments_are_resolved_not_text_presence() -> TestResult {
    ...
    for (old, new) in [
        ...
        ("pad = pad,", "pad = strides,"),
        (
            "interleave = concat_interleave",
            "interleave = missing_bool",
        ),
...
fn reshape_uses_its_bound_shape_constant_and_descriptors_fail_without_panics() -> TestResult {
    ...
    source.mil = source
        .mil
        .replace("shape = gu_shape", "shape = branch_size");
    assert!(interpret(&source, &input).is_err());
    let source = build_ffn_chunk4(&weights)?;
    for bytes in [
        vec![],
        vec![0; 63],
        source.blob[..source.blob.len() - 1].to_vec(),
    ] {
...
fn declared_dtypes_and_positive_finite_scales_are_part_of_the_graph_contract() -> TestResult {
    ...
    for (old, new) in [
        (
            "quantized_data = tensor<int8,",
            "quantized_data = tensor<uint8,",
        ),
        ("zero_point = int8(0)", "zero_point = int8(1)"),
        (
            "tensor<fp16, [1, 32, 1, 1]> gate0",
            "tensor<fp32, [1, 32, 1, 1]> gate0",
        ),
    ] {
        assert!(original.contains(old));
        source.mil = original.replace(old, new);
        assert!(
            interpret(&source, &input).is_err(),
            "accepted dtype mutant {old}"
        );
    }
    source.mil = original;
    let descriptor = source
        .mil
        .lines()
        .find(|line| line.contains("Wg0 = constexpr_"))
        .ok_or("weight")?;
    let offsets: Vec<_> = descriptor
        .split("offset = uint64(")
        .skip(1)
