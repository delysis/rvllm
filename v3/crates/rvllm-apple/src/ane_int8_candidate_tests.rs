...
fn number(text: &str) -> TestResult<usize> {
    text.trim().parse().map_err(|error| format!("number: {error}"))
}
...
fn payload(blob: &[u8], offset: usize, dtype: u32) -> TestResult<&[u8]> {
    let read = |first: usize, bytes: usize| -> TestResult<&[u8]> {
        ...
        blob.get(first..last).ok_or_else(|| "truncated descriptor".into())
    };
    if offset % 64 != 0 || read(offset, 4)? != 0xDEAD_BEEF_u32.to_le_bytes()
    ...
    if pos != offset + 64 || len == 0 || len % 64 != 0 { return Err("bad payload".into()); }
    read(pos, len)
}

fn dimensions(line: &str) -> TestResult<Vec<usize>> {
    let after = line.split_once('[').ok_or("missing shape")?.1;
    after.split_once(']').ok_or("missing shape end")?.0.split(',').map(number).collect()
}

fn argument<'a>(expression: &'a str, name: &str) -> TestResult<&'a str> {
    let marker = format!("{name} = ");
    let tail = expression.split_once(&marker).ok_or_else(|| format!("missing {name}"))?.1;
    Ok(tail.split([',', ')']).next().ok_or("missing argument")?.trim())
}

fn decode_constants(mil: &str, blob: &[u8]) -> TestResult<BTreeMap<String, (usize, usize, Vec<f16>)>> {
    ...
    for line in mil.lines().filter(|line| line.contains(" = constexpr_affine_dequantize()")) {
        ...
        if shape.len() != 4 || shape[2..] != [1, 1] { return Err("weight rank".into()); }
        ...
        let values = q.chunks_exact(shape[1]).zip(scales.chunks_exact(2))
            .flat_map(|(row, scale)| {
                let scale = f16::from_le_bytes([scale[0], scale[1]]).to_f32();
                row.iter().map(move |byte| f16::from_f32(f32::from(i8::from_le_bytes([*byte])) * scale))
            }).collect();
...
fn projection(weights: &[f16], input: &[f16]) -> Vec<f16> {
    weights.chunks_exact(input.len()).map(|row| {
        let sum = row.iter().zip(input).fold(0.0_f32, |sum, (a, b)| sum + a.to_f32() * b.to_f32());
        f16::from_f32(sum)
    }).collect()
}
...
fn interpret(source: &Int8CandidateSource, input: &[f16]) -> TestResult<Vec<f16>> {
    ...
    for line in source.mil.lines() {
        let Some((left, expression)) = line.split_once(" = ") else { continue };
        let name = left.split_whitespace().last().ok_or("name")?;
        if let Some((_, after)) = expression.split_once("val = fp16(") {
            let scalar = after.split(')').next().ok_or("scalar")?.parse::<f32>()
                .map_err(|error| format!("scalar: {error}"))?;
            values.insert(name.into(), vec![f16::from_f32(scalar)]);
            continue;
        }
        if expression.starts_with("conv(") {
            let w = weights.get(argument(expression, "weight")?).ok_or("weight lookup")?;
            let x = values.get(argument(expression, "x")?).ok_or("input lookup")?;
            if x.len() != w.1 { return Err("projection input width".into()); }
            ...
            let names = expression.split_once("values = (").ok_or("concat inputs")?.1
                .split_once(')').ok_or("concat inputs end")?.0;
        ...
        } else if expression.starts_with("mul(") || expression.starts_with("add(") || expression.starts_with("tanh(") {
            let x = values.get(argument(expression, "x")?).ok_or("x lookup")?;
            let y = if expression.starts_with("tanh(") { None }
                else { Some(values.get(argument(expression, "y")?).ok_or("y lookup")?) };
            if y.is_some_and(|y| y.len() != 1 && y.len() != x.len()) { return Err("broadcast width".into()); }
            let output = x.iter().enumerate().map(|(index, x)| {
                let x = x.to_f32();
                let y = y.map(|y| y[if y.len() == 1 { 0 } else { index }].to_f32()).unwrap_or(0.0);
                f16::from_f32(if expression.starts_with("mul(") { x * y }
                    else if expression.starts_with("add(") { x + y } else { x.tanh() })
            }).collect::<Vec<_>>();
            values.insert(name.into(), output);
        } else { continue; }
        let dims = dimensions(line)?;
        let count = dims.iter().try_fold(1_usize, |n, d| n.checked_mul(*d)).ok_or("shape product")?;
        if values.get(name).ok_or("output lookup")?.len() != count { return Err("declared output shape".into()); }
    }
...
fn fixture_weights() -> TestResult<AneInt8FfnWeights> {
    let make = |phase: usize| (0..32 * 128).map(|n| {
        f16::from_f32(((n * 13 + phase) % 97) as f32 / 256.0 - 0.1875)
    }).collect::<Vec<_>>();
...
fn chunk4_generated_graph_and_blob_agree_with_independent_ffn() -> TestResult {
    ...
    for sample in 0..7 {
        let input = (0..32).map(|n| f16::from_f32(((n * 7 + sample) % 23) as f32 / 32.0 - 0.3)).collect::<Vec<_>>();
        let g = projection(&gate, &input);
        let u = projection(&up, &input);
        let gated = g.into_iter().zip(u).map(|(g, u)| activation(g, u)).collect::<Vec<_>>();
...
fn chunk4_preserves_baseline_gelu_order_and_rejects_broken_descriptors() -> TestResult {
    ...
    let baseline = include_str!("ane_linear.rs").split_once("fn ffn_mil_with_projections(")
        .ok_or("baseline producer")?.1.split_once("#[cfg(test)]").ok_or("baseline end")?.0;
    let fixture = baseline.split_once("r#\"").ok_or("baseline raw format")?.1;
    for part in 0..4 {
        let mut previous = 0;
        for line in fixture.lines().filter(|l| l.contains(" = mul(") || l.contains(" = add(") || l.contains(" = tanh(")) {
            let name = line.split_once(" = ").ok_or("fixture assignment")?.0.split_whitespace().last().ok_or("fixture name")?;
    ...
    let header = fixture.split_once("    func main").ok_or("baseline header")?.0
        .replace("{{", "{").replace("}}", "}");
...
fn tiles4_preserves_serialized_rows_and_projection_numerics() -> TestResult {
    let dense = (0..32 * 128).map(|n| f16::from_f32(((n * 29) % 113) as f32 / 256.0 - 0.2))
        .collect::<Vec<_>>();
    ...
    for part in 0..4 {
        assert_eq!(decoded.get(&format!("W{part}")).ok_or("tile")?.2,
            expected[part * 1024..(part + 1) * 1024]);
    }
    for seed in 0..7 {
        let input = (0..32).map(|n| f16::from_f32(((n + seed) % 17) as f32 / 32.0)).collect::<Vec<_>>();
    ...
    corrupt.mil = corrupt.mil.replace("values = (y0, y1, y2, y3)", "values = (y3, y1, y2, y0)");
...
fn graph_shapes_are_narrow_and_do_not_admit_global_qkv() {
    ...
    for shape in [(3840, 8704), (3840, 9216), (3840, 262144), (32, 128), (0, 0),
        (usize::MAX, 15360), (3840, usize::MAX), (3841, 15360), (3840, 15361)] {
        assert!(!linear_tiles4_shape(shape));
        assert!(!ffn_chunk4_shape(shape));
    }
}
...
fn raw_constant<'a>(source: &'a Int8CandidateSource, name: &str) -> TestResult<(&'a [u8], &'a [u8])> {
    let marker = format!("> {name} = constexpr_affine_dequantize()");
    let line = source.mil.lines().find(|line| line.contains(&marker)).ok_or("raw constant")?;
    let offsets = line.split("offset = uint64(").skip(1)
        .map(|text| number(text.split(')').next().unwrap_or("")))
        .collect::<Result<Vec<_>, _>>()?;
    if offsets.len() != 2 { return Err("raw payload count".into()); }
    Ok((payload(&source.blob, offsets[0], 4)?, payload(&source.blob, offsets[1], 1)?))
}
...
fn chunking_is_bit_exact_for_quantized_payloads_and_scale_payloads() -> TestResult {
    ...
    for (prefix, matrix) in [("Wg", gate), ("Wu", up)] {
        ...
        assert_eq!(q, matrix.values.iter().map(|v| v.to_le_bytes()[0]).collect::<Vec<_>>());
        assert_eq!(scales, matrix.scales.iter().flat_map(|s| s.to_le_bytes()).collect::<Vec<_>>());
    }
    let (q, scales) = raw_constant(&source, "Wd")?;
    assert_eq!(q, down.values.iter().map(|v| v.to_le_bytes()[0]).collect::<Vec<_>>());
    assert_eq!(scales, down.scales.iter().flat_map(|s| s.to_le_bytes()).collect::<Vec<_>>());
    Ok(())
}
