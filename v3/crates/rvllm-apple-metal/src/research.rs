...
impl Gemma12bResearchShape {
    pub fn supports(self, candidate: MetalResearchCandidate) -> bool {
        self.hidden == 3840
            ...
            && (candidate == MetalResearchCandidate::ShortMma16x64
...
mod tests {
    ...
    fn model(tokens: u32) -> Gemma12bResearchShape {
        Gemma12bResearchShape { tokens, hidden: 3840, intermediate: 15360,
            layers: 48, heads: 16, kv_heads: 8, head_dim: 256,
            attention_window: 1024, moe_experts: 0, moe_top_k: 0,
            moe_intermediate: 0, ple: 0 }
    }

    #[test]
    fn selection_is_exact_and_default_off() {
        assert_eq!(MetalResearchCandidate::default(), MetalResearchCandidate::Off);
        for kind in [MetalResearchCandidate::Off, MetalResearchCandidate::ShortMma16x64,
            MetalResearchCandidate::RoundedGate32, MetalResearchCandidate::GqaKv8] {
            assert_eq!(kind.name().parse(), Ok(kind));
        }
        for bad in ["", "auto", "mma32", "metal-short-mma16x64 ", "METAL-GQA-KV8"] {
    ...
    fn near_miss_models_never_enter_any_candidate() {
        for kind in [MetalResearchCandidate::ShortMma16x64,
            MetalResearchCandidate::RoundedGate32, MetalResearchCandidate::GqaKv8] {
            let good = model(64);
            assert!(good.supports(kind));
            for bad in [Gemma12bResearchShape { hidden: 4096, ..good },
                Gemma12bResearchShape { layers: 47, ..good },
                Gemma12bResearchShape { heads: 32, ..good },
                Gemma12bResearchShape { kv_heads: 4, ..good },
                Gemma12bResearchShape { head_dim: 128, ..good },
                Gemma12bResearchShape { intermediate: 16384, ..good },
                Gemma12bResearchShape { moe_experts: 1, ..good },
                Gemma12bResearchShape { moe_top_k: 1, ..good },
                Gemma12bResearchShape { moe_intermediate: 1, ..good },
                Gemma12bResearchShape { ple: 1, ..good },
                Gemma12bResearchShape { tokens: u32::MAX, ..good }] {
                assert!(!bad.supports(kind));
            }
            assert_eq!(Gemma12bResearchShape { attention_window: 512, ..good }.supports(kind),
                kind == MetalResearchCandidate::ShortMma16x64);
            assert!(Gemma12bResearchShape {
                kv_heads: 1, head_dim: 512, attention_window: 0, ..good
            }.supports(kind));
        }
    ...
    fn resource_and_alias_guards_reject_boundary_failures() {
        ...
        for (width, max_threads, static_bytes, device_bytes) in
            [(64, 128, 16384, 32768), (32, 127, 16384, 32768),
             (32, 128, 32769, 32768), (32, 128, 8192, 16383)] {
            assert!(!launch_fits(width, max_threads, static_bytes, device_bytes, 128, 16384));
        }
...
pub fn projection_buffers_fit(
    offsets: [usize; 3], shape: [u32; 3], output_bytes: usize, arena_bytes: usize,
) -> bool {
    let [m, n, k] = shape;
    let checked = || {
        if offsets[2] % output_bytes != 0 { return None; }
...
mod short_mma_tests {
    ...
    fn emitted_storage_and_output_abis_are_explicit() {
        ...
        let options = MetalKernelOptions { research: candidate, ..MetalKernelOptions::default() };
        ...
        let ordinary = crate::kernels::kernel_source_with_options(
            MetalFloatType::F16, MetalKernelOptions::default());
        assert_eq!(ordinary.as_ref(), crate::kernels::KERNEL_SOURCE);
        assert!(!crate::kernels::KERNEL_NAMES.iter().any(|name| name.starts_with("research_")));
    }
    ...
    fn cooperative_staging_and_fragment_output_cover_each_element_once() {
        ...
        for tid in 0..128 {
            for i in (tid..512).step_by(128) { a_writes[(i / 32) * 40 + i % 32] += 1; }
            for i in (tid..2048).step_by(128) { b_writes[(i / 32) * 40 + i % 32] += 1; }
        }
        ...
        for sg in 0..4 {
            for row in 0..16 {
                for col in 0..16 { output[row * 64 + sg * 16 + col] += 1; }
            }
        }
...
pub fn rounded_gate_buffers_fit(
    offsets: [usize; 4], tokens: u32, hidden: u32, intermediate: u32,
    arena_bytes: usize, capture_gate_up: bool,
) -> bool {
    let checked = || {
        let two_intermediate = intermediate.checked_mul(2)?;
        let a = buffer_span(offsets[0], matrix_bytes(tokens, hidden, 2)?, arena_bytes)?;
        let w = buffer_span(offsets[1], matrix_bytes(two_intermediate, hidden, 2)?, arena_bytes)?;
        let gu = buffer_span(offsets[2], matrix_bytes(tokens, two_intermediate, 2)?, arena_bytes)?;
        let out = buffer_span(offsets[3], matrix_bytes(tokens, intermediate, 2)?, arena_bytes)?;
        Some(if capture_gate_up { disjoint_writes(&[a, w], &[gu, out]) }
            else { disjoint_writes(&[a, w], &[out]) })
    };
...
mod rounded_gate_tests {
    ...
    fn fusion_rejects_shortened_input_or_weight_lifetimes() {
        ...
        assert!(rounded_gate_buffers_fit([0, 16, 64, 88], 2, 4, 3, 100, true));
        assert!(!rounded_gate_buffers_fit([0, 16, 64, 88], 2, 4, 3, 99, true));
        assert!(!rounded_gate_buffers_fit([0, 16, 64, 8], 2, 4, 3, 100, false));
        assert!(!rounded_gate_buffers_fit([0, 16, 64, 32], 2, 4, 3, 100, false));
        assert!(!rounded_gate_buffers_fit([0, 16, 64, 80], 2, 4, 3, 100, true));
        assert!(rounded_gate_buffers_fit([0, 16, 64, 80], 2, 4, 3, 100, false));
        assert!(!rounded_gate_buffers_fit([0, 16, 64, 88], 2, 4, u32::MAX, usize::MAX, false));
    }

    #[test]
    fn generated_epilogue_retains_both_storage_rounding_boundaries() {
        for dtype in [MetalFloatType::F16, MetalFloatType::Bf16] {
            let source = crate::kernels::kernel_source_with_options(dtype, MetalKernelOptions {
                research: MetalResearchCandidate::RoundedGate32, ..MetalKernelOptions::default()
            });
            let extra = source.split("// metal-rounded-gate32:").nth(1).unwrap();
            let storage = if dtype == MetalFloatType::F16 { "half" } else { "bfloat" };
            let round = if dtype == MetalFloatType::F16 { "f16_sat" } else { "bf16_sat" };
            assert!(extra.contains(&format!("{storage} rounded_gate = {round}(cg[index] + 0.0f)")));
            assert!(extra.contains(&format!("{storage} rounded_up = {round}(cu[index] + 0.0f)")));
            assert!(extra.contains(&format!("{round}(gelu_tanh(float(rounded_gate)) * float(rounded_up))")));
    ...
    fn removed_bf16_projection_rounding_is_observably_a_different_algorithm() {
        ...
        for i in -80..80 {
            ...
            let u = 0.731_f32;
            let stored = bf16::from_f32(gelu(bf16::from_f32(g).to_f32()) * bf16::from_f32(u).to_f32());
            let unrounded = bf16::from_f32(gelu(g) * u);
            if stored.to_bits() != unrounded.to_bits() { witnesses += 1; }
        }
        assert!(witnesses > 0, "the roundings must not be optimized out of the source contract");
    }
}
    ...
    pub fn buffers_fit(self, offsets: [usize; 8], arena_bytes: usize) -> bool {
        let checked = || {
            if !matches!((self.kv_heads, self.head_dim), (8, 256) | (1, 512))
                || self.block_size == 0 || self.max_blocks == 0 || self.num_blocks == 0
            ...
            let cache_bytes = matrix_bytes(self.num_blocks.checked_mul(self.block_size)?,
                self.kv_heads.checked_mul(self.head_dim)?, 2)?;
            ...
            let table = buffer_span(offsets[4], matrix_bytes(self.max_blocks, 1, 4)?, arena_bytes)?;
            ...
            Some(disjoint_writes(&[q, k, v, table, lengths, cu, positions], &[out]))
        };
        checked().unwrap_or(false)
    }
}
...
mod gqa_tests {
        ...
        for (kv_heads, head_dim, threads, scratch) in [(8, 256, 64, 8224), (1, 512, 128, 16416)] {
            ...
            for kv in 0..kv_heads {
                for chunk in 0..(gqa + 3) / 4 {
                    for sg in 0..threads / 32 { heads[kv * gqa + chunk * 4 + sg] += 1; }
                }
            }
    ...
    fn metadata_and_kv_bounds_are_checked_including_output_aliases() {
        let shape = GqaBufferShape { tokens: 1, kv_heads: 1, head_dim: 512,
            block_size: 32, max_blocks: 1, num_blocks: 1 };
        ...
        assert!(!GqaBufferShape { max_blocks: u32::MAX, ..shape }.buffers_fit(offsets, usize::MAX));
        assert!(!GqaBufferShape { num_blocks: 0, ..shape }.buffers_fit(offsets, usize::MAX));
    }
    ...
    fn source_keeps_online_statistics_float_and_checks_physical_pages() {
        ...
        assert!(source.contains("out_lane[slot] = out_lane[slot] * correction + weight * float(vt[j * D + d])"));
