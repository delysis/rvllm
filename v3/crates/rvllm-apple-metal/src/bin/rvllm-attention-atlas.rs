...
fn run() -> Result<()> {
    ...
    match a.iter().map(String::as_str).collect::<Vec<_>>().as_slice() {
        ...
        ["trial", prepared, out] => trial(Path::new(prepared), Path::new(out)),
...
#[cfg(target_os = "macos")]
fn trial(prepared: &Path, out: &Path) -> Result<()> {
    e::fresh(out)?;
    let build = out.join("build");
    let oracle = out.join("oracle");
    let bench = out.join("bench");
    e::compile(prepared, &build)?;
    atlas::metal::run(prepared, &build, None, &oracle)?;
    atlas::metal::run(prepared, &build, Some(&oracle.join("oracle.json")), &bench)
}
#[cfg(not(target_os = "macos"))]
fn trial(_: &Path, _: &Path) -> Result<()> {
    Err(Error::new(
