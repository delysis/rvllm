...
#[path = "rvllm_ane_int8_probe/batch_timing.rs"]
mod batch_timing;

#[path = "rvllm_ane_int8_probe/interleaved_timing.rs"]
mod interleaved_timing;
...
#[path = "rvllm_ane_int8_probe/capture_journal.rs"]
mod capture_journal;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut exploratory_fair = false;
    let mut interleaved_timing = false;
    ...
    while let Some(flag) = args.next() {
        ...
        match flag.as_str() {
            ...
            "--exploratory-fair" => exploratory_fair = value.parse()?,
            "--interleaved-timing" => interleaved_timing = value.parse()?,
    ...
    if (exploratory_fair || interleaved_timing)
        && (mode != Mode::Int8
            || int8_batch != 2
            || !compare
            || cpu_only
            ...
            || compile_budget != 0
            || std::env::var_os("RVLLM_ANE_DIAGNOSTIC_JOURNAL").is_some())
    {
        return Err("Fair/interleaved exploration requires only two-token INT8 timing, cache require, zero compilation and no driver journal".into());
    }
    ...
    if restore_int8_cache
        && (mode != Mode::Int8
            ...
            || std::env::var_os("RVLLM_ANE_DIAGNOSTIC_JOURNAL").is_none())
    {
        return Err("INT8 cache restoration requires mode int8, cache reuse, compile budget 1, driver journal, and no CPU/comparison/stacked/palette experiment; only S1 or the existing S2 graph is supported".into());
    }
    ...
    if int8_layout_experiment && !restore_int8_cache {
        let QuantizedWeights::Int8(weights) = &quantized else {
            unreachable!()
        };
    ...
    if restore_int8_cache {
        ...
        // Restoration visits exactly the selected existing graph shape. It
        // never evaluates S1 as a prelude to restoring S2, and never combines
        // layouts or changes the already qualified INT8 coefficients.
        let restored = if int8_batch == 2 {
            AneGatedFfn::compile_int8_batch_with_cache_policy(
                weights,
                2,
                AneProgramCachePolicy::ReuseOrCompileUpTo(1),
            )?
        } else {
            AneGatedFfn::compile_int8_with_cache_policy(
                weights,
                AneProgramCachePolicy::ReuseOrCompileUpTo(1),
            )?
        };
        ...
        let result = json!({"schema":"rvllm.int8_ffn_cache_restore.v1","model_dir":model_dir,"layer":layer,
            "original_fp16_sha256":original_hash,"dense_reconstruction_fp16_sha256":reconstructed_hash,
            "logical_tokens":int8_batch.max(1),"affine_coefficients_sha256":ffn_oracle::affine_hash(weights),
