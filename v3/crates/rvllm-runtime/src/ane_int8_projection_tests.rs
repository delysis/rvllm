...
#[path = "ane_int8_global_split_tests.rs"]
mod global_split;
...
fn load_qkv_inputs(
    ...
    layer: usize,
    sources: &[PathBuf],
) -> Result<QkvInputs, Box<dyn std::error::Error>> {
    if !(1..LAYERS).contains(&layer) || sources.is_empty() || sources.len() > 8 {
        return Err("requires layer 1..47 and 1..8 captures".into());
    }
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let QkvInputs {
        ...
        inputs,
        input_receipts,
    } = load_qkv_inputs(&model, layer, &sources)?;
    let rows = q + kv * if shared_value { 1 } else { 2 };
