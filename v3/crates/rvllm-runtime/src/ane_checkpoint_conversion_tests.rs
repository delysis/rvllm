...
fn all_half_encodings_match_scalar_conversion_or_error() {
    for dtype in [DType::Bf16, DType::F16] {
        ...
        for bits in 0..=u16::MAX {
            ...
            let expected = decode_f16_scalar(&input, dtype, &mut scalar);
            let actual = decode_f16(&input, dtype, &mut candidate);
        ...
        // qualify only the compiler's scalar tail.
        let mut batch = vec![f16::ZERO; finite_expected.len()];
        decode_f16(&finite_bytes, dtype, &mut batch).unwrap();
        assert!(batch
            .iter()
            .zip(finite_expected)
...
#[test]
fn conversion_handles_chunk_tails_and_length_errors() {
    for length in [0, 1, 7, 1023, 1024, 1025, 3840, 15360] {
        ...
        decode_f16_scalar(&input, DType::Bf16, &mut scalar).unwrap();
        decode_f16(&input, DType::Bf16, &mut candidate).unwrap();
        assert!(scalar
            .iter()
            .zip(&candidate)
    ...
    for (input, length, dtype) in [
        ...
        assert_eq!(
            decode_f16_scalar(&input, dtype, &mut vec![f16::ZERO; length]),
...
fn checkpoint_conversion_abba() {
    ...
    decode_f16_scalar(&bytes, entry.dtype, &mut reference).unwrap();
    decode_f16(&bytes, entry.dtype, &mut converted).unwrap();
