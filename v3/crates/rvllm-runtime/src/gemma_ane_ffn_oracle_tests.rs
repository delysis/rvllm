...
#[derive(Clone, Copy)]
enum Candidate { Chunk4, Down4, Interleaved }
...
fn validate_input(raw: &[u8], candidate: Candidate) -> Result<Input, String> {
    ...
    if input.schema != SCHEMA || input.candidate != candidate.name()
        || input.layer >= LAYERS || !(1..=3).contains(&input.samples.len())
        || !valid_hash(&input.model_config_sha256)
        || input.quantized_matrix_sha256.iter().any(|h| !valid_hash(h))
        || input.samples.iter().any(|s| !s.path.is_absolute() || !valid_hash(&s.sha256))
    {
        return Err("FFN component manifest shape, candidate or pins invalid".into());
    }
    for (i, sample) in input.samples.iter().enumerate() {
        if input.samples[..i].iter().any(|s| s.path == sample.path || s.sha256 == sample.sha256) {
...
fn regular_bytes(path: &Path, limit: usize) -> Result<Vec<u8>, String> {
    ...
    if bytes.len() > limit { return Err("component input exceeds size bound".into()); }
    Ok(bytes)
}

fn pinned_bytes(path: &Path, expected: &str, limit: usize) -> Result<Vec<u8>, String> {
    if !valid_hash(expected) { return Err("invalid input SHA256".into()); }
    let bytes = regular_bytes(path, limit)?;
    if sha(&bytes) != expected { return Err("component input hash changed".into()); }
    Ok(bytes)
}

fn half_input(bytes: &[u8]) -> Result<Vec<f16>, String> {
    if bytes.len() != HIDDEN * 2 { return Err("FFN input must contain exactly 3840 f16 values".into()); }
    let values: Vec<_> = bytes.chunks_exact(2).map(|b| f16::from_le_bytes([b[0], b[1]])).collect();
    if values.iter().any(|v| !v.is_finite()) { return Err("nonfinite captured FFN input".into()); }
    Ok(values)
}
...
fn matrix_hash(matrix: rvllm_apple::ane_int8_ffn_weights::AneInt8MatrixView<'_>) -> String {
    ...
    for values in matrix.values.chunks(block.len()) {
        for (dst, value) in block.iter_mut().zip(values) { *dst = value.to_le_bytes()[0]; }
        digest.update(&block[..values.len()]);
    }
    for scale in matrix.scales { digest.update(scale.to_le_bytes()); }
...
fn output(directory: &Path, name: &str, values: &[f16]) -> Result<String, String> {
    let mut file = std::fs::OpenOptions::new().write(true).create_new(true)
        .open(directory.join(name)).map_err(|e| e.to_string())?;
...
fn load_component_weights(
    ...
    if layer >= LAYERS { return Err("component layer out of range".into()); }
    ...
    let load = |name: &str| -> Result<Vec<f16>, String> {
        load_tensor(entries.get(&format!("{prefix}.{name}_proj.weight")).ok_or("missing FFN weight")?)
    };
    let gate = load("gate")?;
    let up = load("up")?;
    let down = load("down")?;
...
fn run(candidate: Candidate) -> Result<(), String> {
    ...
    if [&model_dir, &manifest, &directory, &journal].iter().any(|p| !p.is_absolute()) {
    ...
    for sample in &input.samples {
        samples.push(half_input(&pinned_bytes(&sample.path, &sample.sha256, HIDDEN * 2)?)?);
    }
    ...
    if observed != input.quantized_matrix_sha256 { return Err("FFN coefficient pins disagree".into()); }
    std::fs::create_dir(&directory).map_err(|e| e.to_string())?;
    let mut events = std::fs::OpenOptions::new().write(true).create_new(true)
        .open(directory.join("events.jsonl")).map_err(|e| e.to_string())?;
    record(&mut events, serde_json::json!({"event":"begin", "schema":SCHEMA,
    ...
    let result = (|| -> Result<(), String> {
        ...
        record(&mut events, serde_json::json!({"event":"load-control-begin"}))?;
        let mut control = match candidate {
            Candidate::Interleaved => AneGatedFfn::compile_int8_stacked_with_cache_policy(&weights, policy)?,
            _ => AneGatedFfn::compile_int8_with_cache_policy(&weights, policy)?,
        };
        record(&mut events, serde_json::json!({"event":"load-candidate-begin"}))?;
        let mut proposed = match candidate {
            Candidate::Chunk4 => AneGatedFfn::compile_int8_chunk4_with_cache_policy(&weights, policy)?,
            Candidate::Down4 => AneGatedFfn::compile_int8_down4_with_cache_policy(&weights, policy)?,
            Candidate::Interleaved => AneGatedFfn::compile_int8_interleaved_with_cache_policy(&weights, policy)?,
        };
        let mut a = vec![f16::ZERO; HIDDEN];
        let mut b = vec![f16::ZERO; HIDDEN];
        for (index, sample) in samples.iter().enumerate() {
            record(&mut events, serde_json::json!({"event":"control-evaluate-begin","sample":index}))?;
            control.project(sample, &mut a)?;
            let a_hash = output(&directory, &format!("sample-{index}-control.f16"), &a)?;
            record(&mut events, serde_json::json!({"event":"candidate-evaluate-begin","sample":index,
            ...
            record(&mut events, serde_json::json!({"event":"comparison","sample":index,
    ...
    record(&mut events, serde_json::json!({"event":"end", "matched_all_inputs":result.is_ok(),
...
fn validate_pin_request(bytes: &[u8]) -> Result<(PinRequest, Candidate), String> {
    ...
    if request.schema != "rvllm.ane.ffn-pin-request.v1" || request.layer >= LAYERS
        || !(1..=3).contains(&request.sample_paths.len())
        || request.sample_paths.iter().any(|path| !path.is_absolute())
        || request.sample_paths.iter().enumerate().any(|(i, path)| request.sample_paths[..i].contains(path))
    {
...
fn prepare_input_pins() -> Result<(), String> {
    ...
    if [&model_dir, &request_path, &directory].iter().any(|p| !p.is_absolute()) {
    ...
    file.write_all(&manifest_bytes).and_then(|_| file.sync_all()).map_err(|e| e.to_string())?;
    ...
    let mut receipt = File::create_new(directory.join("host-pin-receipt.json"))
...
fn host_prepare_ffn_component_pins() -> Result<(), String> { prepare_input_pins() }

#[test]
#[ignore = "explicit cached-only real-input ANE chunk4 component oracle; no timing"]
fn native_chunk4_matches_plain_cached_ffn() -> Result<(), String> { run(Candidate::Chunk4) }

#[test]
#[ignore = "explicit cached-only real-input ANE down4 component oracle; no timing"]
fn native_down4_matches_plain_cached_ffn() -> Result<(), String> { run(Candidate::Down4) }

#[test]
#[ignore = "explicit cached-only real-input ANE interleaved component oracle; no timing"]
fn native_interleaved_matches_stacked_cached_ffn() -> Result<(), String> { run(Candidate::Interleaved) }
...
mod tests {
    ...
    fn host_pin_request_is_bounded_and_cannot_select_an_unknown_or_duplicate_case() {
        ...
        let parse = |value: &serde_json::Value| validate_pin_request(&serde_json::to_vec(value).unwrap());
        assert!(parse(&request).is_ok());
        for (key, value) in [("candidate", serde_json::json!("auto")),
            ("layer", serde_json::json!(48)), ("sample_paths", serde_json::json!([])),
            ("sample_paths", serde_json::json!(["/same", "/same"])),
            ("sample_paths", serde_json::json!(["relative.f16"]))] {
            let mut mutant = request.clone(); mutant[key] = value;
    ...
    fn manifest_refuses_wrong_candidate_unbounded_samples_and_unpinned_coefficients() {
        let parse = |v: &serde_json::Value| validate_input(&serde_json::to_vec(v).unwrap(), Candidate::Interleaved);
        let good = fixture();
        assert!(parse(&good).is_ok());
        for (key, bad) in [("layer",serde_json::json!(48)),("candidate",serde_json::json!("ane-int8-ffn-down4")),
            ("samples",serde_json::json!([])),("quantized_matrix_sha256",serde_json::json!([null,null,null]))] {
            let mut mutant=fixture(); mutant[key]=bad; assert!(parse(&mutant).is_err());
        }
        let mut duplicate=fixture(); let row=duplicate["samples"][0].clone();
        duplicate["samples"]=serde_json::json!([row.clone(),row]); assert!(parse(&duplicate).is_err());
        let duplicate_key=String::from_utf8(serde_json::to_vec(&good).unwrap()).unwrap()
            .replacen('{',"{\"layer\":0,",1);
    ...
    fn captured_input_is_exact_width_finite_and_signed_zero_preserving() {
        let mut bytes=vec![0_u8;HIDDEN*2]; bytes[..2].copy_from_slice(&0x8000_u16.to_le_bytes());
        assert_eq!(half_input(&bytes).unwrap()[0].to_bits(),0x8000);
        assert!(half_input(&bytes[..bytes.len()-1]).is_err());
        bytes[..2].copy_from_slice(&0x7c00_u16.to_le_bytes()); assert!(half_input(&bytes).is_err());
    }
    #[test]
    fn matrix_identity_hash_binds_shape_coefficients_and_scales_in_order() {
        ...
        let values=[-2_i8,7,1,-1]; let scales=[f16::ONE,f16::from_f32(0.5)];
        let view=||AneInt8MatrixView{columns:2,values:&values,scales:&scales};
        let mut bytes=2_u64.to_le_bytes().to_vec(); bytes.extend(2_u64.to_le_bytes());
        bytes.extend([254_u8,7,1,255]); bytes.extend([0_u8,60,0,56]);
        assert_eq!(matrix_hash(view()),sha(&bytes));
        assert_ne!(matrix_hash(AneInt8MatrixView{columns:4,..view()}),sha(&bytes));
    }
}
